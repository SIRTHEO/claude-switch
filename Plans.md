...
| 3a.4 | Linux behaviour spike (Docker container) | Findings in `.claude/docs/reports/profiles-linux.md`: tokens land in `<profile>/.claude.json`, no Keychain assumption; edge cases listed | 3a.1 | cc:完了 [0363150] |
...
| 4.2 | Coverage gap analysis on `src/auto-fallback.ts`, `src/fallback.ts`, `src/apikey.ts` | Per-function coverage report; missing edge cases listed | 0.5, 4.1 | cc:完了 |
| 4.2-result | **All 4 fallback-related modules at 100% line coverage** as of HEAD: fallback.ts (100/100/100), auto-fallback.ts (100/91/100), apikey.ts (100/96/100), fallback-env.ts (100/100/100). Branch coverage on auto-fallback.ts (91%) is the highest realistic — remaining branches are catch arms in malformed-config paths. The only fallback-adjacent coverage gap is `ui/set-apikey.ts` (10%), which is TUI territory and belongs to Phase 5's TTY-mocking work, not Phase 4 | - | - |
...
| 6.5 | Merge + verify release-please publishes 2.7.0 | npm package live, GitHub Release created, install command works on a clean machine | 6.4 | cc:完了 [2.7.0] |
...
| 7.6 | Push + merge release-please PR → 2.8.0 on npm | npm package live; 2 feat commits → minor bump | 7.1–7.4 | cc:完了 [2.8.0] |
