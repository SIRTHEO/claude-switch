...
describe('save/load — API-key acceptance leak prevention', () => {
  ...
  it('clears customApiKeyResponses + apiKey on load when target has no snapshot', () => {
    ...
    fs.writeFileSync(path.join(accDir, 'sirtheo@example.com.json'), JSON.stringify({
      emailAddress: 'sirtheo@example.com',
    }));
    load('sirtheo@example.com', claudeJson, accDir);
    const after = JSON.parse(fs.readFileSync(claudeJson, 'utf-8'));
    assert.equal(after.oauthAccount.emailAddress, 'sirtheo@example.com');
  ...
  it('restores customApiKeyResponses + apiKey on load when target has its own snapshot', () => {
    ...
    fs.writeFileSync(path.join(accDir, 'sirtheo@example.com.json'), JSON.stringify({
      emailAddress: 'sirtheo@example.com',
      _customApiKeyResponses: { approved: ['sk-ant-api03-sirtheo'], rejected: [] },
      _claudeJsonApiKey: 'sk-ant-api03-sirtheo',
    }));
    load('sirtheo@example.com', claudeJson, accDir);
    const after = JSON.parse(fs.readFileSync(claudeJson, 'utf-8'));
    assert.deepEqual(after.customApiKeyResponses, { approved: ['sk-ant-api03-sirtheo'], rejected: [] });
    assert.equal(after.apiKey, 'sk-ant-api03-sirtheo');
  });

  it('round-trip: save tech, switch to sirtheo, switch back to tech — tech regains its own approval', () => {
    fs.writeFileSync(claudeJson, JSON.stringify({
      oauthAccount: { emailAddress: 'tech@example.com' },
      customApiKeyResponses: { approved: ['sk-ant-api03-tech'], rejected: [] },
    }));
    ...
    // Switch to sirtheo (no snapshot) — claude.json should be wiped.
    fs.writeFileSync(path.join(accDir, 'sirtheo@example.com.json'), JSON.stringify({
      emailAddress: 'sirtheo@example.com',
    }));
    load('sirtheo@example.com', claudeJson, accDir);
    ...
    save('sirtheo@example.com', claudeJson, accDir);
