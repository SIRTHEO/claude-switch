...
| `~/.claude.json` active account | `sirtheo.personal@example.com` |
| Saved legacy accounts | `sirtheo.personal@example.com.json`, `sirtheo.work@example.com.json` (both have `_keychain` blob) |
...
```
$ node dist/bin/cli.js switch profile import sirtheo.work@example.com --as harness-import-test

✔ Imported "sirtheo.work@example.com" into profile "harness-import-test"
  Path:    /Users/example/.claude/profiles/harness-import-test
...
| `profile list` shows `→ sirtheo.work@example.com` | ✅ |
| `profile status harness-import-test` shows email + full userID | ✅ |

⚠️ **Finding (informational)**: imported tokens for `sirtheo.work@example.com` had `expiresAt` 4 days in the past. This is expected — the legacy `_keychain` blob stores the snapshot from when `claude switch` last touched the account, not a fresh token. Claude Code refreshes via the refreshToken on next run. This needs to be verified in 3a.1b (interactive `profile use`).

## Test 4 — Cleanup + post-state restoration

```
$ node dist/bin/cli.js switch profile remove harness-import-test
Removed profile dir: /Users/example/.claude/profiles/harness-import-test
...
| `~/.claude.json` active account | `sirtheo.personal@example.com` | `sirtheo.personal@example.com` | ✅ |
